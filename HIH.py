        ...
        while now < end_time:
            ...
            try:
                ...
                if Tweets_folder.exists() == False:
                    Tweets_folder.mkdir()
                cwd = os.getcwd()
                header = f'Date_Created~]ID~]User_Name~]Screen_Name~]Tweet_Text~]Geo_lat~]Geo_long~]Location~]Lat~]Long~]City~]Country~]Language~]Follower_Count~]Friend_Count~]Retweet_Count~]Favorite_Count~]Start_time_{now}~]End_time_{end_time}'
